"""Batch extraction helper for processing many documents through one pipeline.
...
Despite the original location in ``idp.llm.nanonets_batch``, this
helper is backend-agnostic: it runs any ``Pipeline`` regardless of
backend. The legacy import path is kept as a thin re-export for
