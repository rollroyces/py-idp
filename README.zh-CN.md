...
**语言:** [English](https://github.com/rollroyces/py-idp/blob/main/README.md) · [繁體中文](https://github.com/rollroyces/py-idp/blob/main/README.zh-TW.md) · [简体中文](https://github.com/rollroyces/py-idp/blob/main/README.zh-CN.md)
·
...
详见 [CHANGELOG.md](https://github.com/rollroyces/py-idp/blob/main/CHANGELOG.md) 查看发布说明。
...
flowchart TB
    ...
    subgraph API["FastAPI 服务 (idp.api)"]
    ...
    subgraph LLM["idp.llm"]
        backends[Backend 协议<br/>+ 可靠性包装]
    end
    subgraph Eval["idp.eval + idp.rl"]
        harness[评测运行器 /<br/>策略更新 /<br/>校准]
    end
    subgraph HITL["idp.hitl"]
        streamlit[Streamlit<br/>复核 UI]
    end
...
每个子包都可以独立 `import idp.<name>` 使用 —— 例如 `idp.core` 不依赖其他包，`idp.llm` 只依赖 `idp.errors`。

---

## LLM 后端

`py-idp` 使用统一的 `Backend` 协议，每个 provider 都实现它。Pipeline 不需要关心 provider 的具体实现 —— 永远通过 `backend.complete(CompletionRequest)` 调用。
...
flowchart LR
    ...
    EX --> Backend[Backend<br/>协议]
    Backend --> Wrap{RetryingBackend<br/>+ ExtractionCache}
    Wrap --> Call[complete<br/>request]
    Call --> P[openai / anthropic /<br/>ollama / china:qwen /<br/>compat: 任意 OpenAI 风格]
    Call --> N[Nanonets-OCR2-3B<br/>自托管 VLM]
...
详见 [`src/idp/chunker.py`](https://github.com/rollroyces/py-idp/blob/main/src/idp/chunker.py) 实现与 [`tests/test_chunker.py`](https://github.com/rollroyces/py-idp/blob/main/tests/test_chunker.py) 34 个测试。
...
idp batch path/to/invoices/ --backend ollama --schema Invoice \    # 批量处理
...
对于需要处理大量文档的场景（Databricks 任务、cron 定时作业、S3 前缀重放），推荐使用 `idp batch` 命令，而不是循环调用 `idp run`：

```bash
idp batch /mnt/inbox/ \                         # 目录：递归扫描 *.pdf/*.png/*.jpg/*.tiff/*.txt
          --backend ollama --schema Invoice \
          --output out.jsonl \                  # 每篇文档一条 JSONL：path, ok, extraction, ...
          --report report.json \                # 聚合：吞吐量、p50/p95 延迟、错误直方图
          --dlq dlq.jsonl \                     # 仅失败文档（用于二次重试）
          --checkpoint ledger.jsonl \           # 续跑账本：重跑时从中断处继续
          --progress-every 50                   # 每 50 篇打一次日志；0 表示静默
```

数据源可以是 glob 模式、显式列表（`@/mnt/inbox.lst`），或路径与目录的任意组合。文件按绝对路径去重。

编程接口：`idp.process_batch` 与 `idp.BatchItemResult` 已导出至顶层 —— `from idp import process_batch` 即可使用。（旧路径 `from idp.llm.nanonets_batch import process_batch` 保留为 12 行 re-export 兼容垫片。）

v0.3.x 中流水线为**串行执行**；并发支持将在未来版本加入。当前的吞吐大致与单篇 `idp run` 相同。
...
详见 [`src/idp/discover.py`](https://github.com/rollroyces/py-idp/blob/main/src/idp/discover.py) 实现，[`tests/test_discover.py`](https://github.com/rollroyces/py-idp/blob/main/tests/test_discover.py) 45 个测试，以及 [`examples/discover_schema_sample.py`](https://github.com/rollroyces/py-idp/blob/main/examples/discover_schema_sample.py) 可运行的端到端示例。
...
```mermaid
flowchart LR
    Ext[extract] --> Schema[Schema 校验<br/>Pydantic 类型]
    Schema --> Rules[business_rules<br/>谓词列表]
...
![py-idp 在示例发票上的流水线](https://github.com/rollroyces/py-idp/blob/main/docs/assets/demo-pipeline.svg)

同一个抽取过程在 Streamlit HITL 复核 UI 中的呈现：

![Streamlit HITL 复核界面](https://github.com/rollroyces/py-idp/blob/main/docs/assets/demo-hitl.svg)
...
```mermaid
flowchart LR
    A[数据集<br/>cases.jsonl + docs/] -->|load| B[评测运行器]
    B -->|对每个后端| C[Pipeline.run<br/>+ extract + assess]
    C --> D{Schema<br/>合法？}
    D -->|是| E[field_match<br/>对比 gold]
    D -->|否| F[标记非法]
    E --> G[聚合<br/>P/R/F1]
    F --> G
    G --> H[results.json<br/>每后端一行]
    H -->|CI 或报告| I[发布或<br/>阻止合并]
```
...
详见 [`src/idp/reliability.py`](https://github.com/rollroyces/py-idp/blob/main/src/idp/reliability.py)、
...
发现安全漏洞？请查阅 [`docs/SECURITY.md`](https://github.com/rollroyces/py-idp/blob/main/docs/SECURITY.md) —— **请勿**以公开 issue 形式提交。

---

## 引用

如果 py-idp 帮到了你的研究或产品，学术引用见 [`CITATION.cff`](https://github.com/rollroyces/py-idp/blob/main/CITATION.cff)。GitHub 侧栏的 "Cite this repository" 按钮可一键导出 BibTeX。

---

## 贡献

欢迎提交 Issue、PR 和 Discussion。完整指南——包括如何新增 LLM 后端或 Schema、提交消息规范、发布流程——见 [`CONTRIBUTING.md`](https://github.com/rollroyces/py-idp/blob/main/CONTRIBUTING.md)。Bug 报告最好附上最小复现脚本和你的 `py-idp` 版本。CI 在每次 PR 上运行 ruff + mypy + 508 个测试，覆盖 Python 3.10 / 3.11 / 3.12。
